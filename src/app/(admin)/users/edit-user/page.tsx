...
import { MUTATION_EDIT_USER, QUERY_USER } from "@src/graphql/user";
...
import { IDetailUserResponse, IEditUserResponse } from "./_interface/edit-user";
...
export default function EditUser() {
  ...
  const {
    ...
    loading,
    refetch,
  } = useQuery<IDetailUserResponse>(QUERY_USER);
  const [mutationEditUser] = useMutation<IEditUserResponse>(MUTATION_EDIT_USER);
  ...
  useEffect(() => {
    !searchParams.get("userId") && router.back();
    refetch({ userId: searchParams.get("userId") });
  });
  ...
  const onSubmit: SubmitHandler<IFormUser> = async (data: IFormUser) => {
    ...
    if (!data.email) delete queryData.email;
    if (!data.phone) delete queryData.phone;

    try {
      const mutate = mutationEditUser({
        variables: {
      ...
      const { message, success } = resp.data?.editProfileUser!;
                    ...
                    <div className="row-span-1 col-span-6 px-2 py-1">
                      <div>
                        ...
                        <select
                          ...
                          defaultValue={userData?.user.userLevel}
                          {...register("userLevel")}
                        >
                    ...
                    <div className="row-span-1 col-span-6 px-2 py-1">
                      <div>
                        ...
                        <input
                          className="input_animation_focus_1 border-neutral-gray-40 rounded-lg px-4 py-2 h-[42px]"
                          type="date"
                          defaultValue={userData?.user.birthDay}
                          {...register("birthDay")}
                          name="birthDay"
                        />
                      </div>
                    ...
                    <div className="row-span-1 col-span-6 px-2 py-1">
                      <div>
                        ...
                        <Controller
                          ...
                          control={control}
                          defaultValue={userData?.user.address?.province}
                    ...
                    <div className="row-span-1 col-span-6 px-2 py-1">
                      <div>
                        ...
                        <Controller
                          ...
                          defaultValue={userData?.user.address?.district ?? ""}
                          render={({ formState: { errors }, field }) => (
                            <div>
                              <AutoCompleteWithOutAdd
                    ...
                    <div className="row-span-1 col-span-6 px-2 py-1">
                      <div>
                        ...
                        <Controller
                          ...
                          name="address.ward"
                          // rules={{ required: "Chọn danh mục là bắt buộc" }}
                          defaultValue={userData?.user.address?.ward ?? ""}
                          render={({ formState: { errors }, field }) => (
                            <div>
                              <AutoCompleteWithOutAdd
